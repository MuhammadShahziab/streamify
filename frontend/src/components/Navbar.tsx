...
const Navbar = () => {
  ...
  const shouldAnimateBell = hasUnreadNotifications && !isNotificationsPage;
          ...
          <div className="flex items-center gap-3 sm:gap-4 ml-auto">
            <Link to={"/notifications"}>
              <button className="btn btn-ghost btn-circle relative">
                <BellIcon
                  className={clsx(
                    "h-6 w-6 text-base-content opacity-70",
                    shouldAnimateBell && "bell-shake"
                  )}
                />
                {shouldAnimateBell && (
                  <span className="w-1 h-1 rounded-full bg-red-600 absolute top-3 right-2 animate-ping " />
                )}
              </button>
            </Link>
          </div>
