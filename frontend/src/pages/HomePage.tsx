...
const HomePage = () => {
  ...
  const {
    ...
    console.log(outGoingFriendReqs ,"check outgoingids")
  ...
  //Send friend Request
  const { mutate: sendRequestMutation ,isPending:pendingSendReq } = useMutation({
    mutationFn: sendFriendReq,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["outgoingFriendReqs"] });
    },
  });
  console.log(outgoingRequestsIds,"outGoingFriendReqs")
   useEffect(() => {
    if (outGoingFriendReqs?.length) {
      const ids = new Set<string>(outGoingFriendReqs.map((f: any) => f.recipient?._id));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {recommendedUsers?.map((user) => {
               const hasRequestBeenSent = outgoingRequestsIds.has(user?._id)
                return(
                <ContactCard key={user?._id} isLoading={pendingSendReq} user={user} hasRequestBeenSent={hasRequestBeenSent} onSendRequest={(id) => sendRequestMutation(id)} />
                );
              })}
            </div>
          )}
